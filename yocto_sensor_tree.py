...
def usage():
    ...
    print("Usage:")
    print(scriptname + ' list ')
    print(scriptname + ' all  ')
    sys.exit()
...
if target == 'all':
    # retreive any humidity sensor
        ...
        if lightT.isOnline() and humidT.isOnline() is True:
                i = 1
                for i in range(1,6):
                	print('%2.2f' % tempT.get_currentValue() + "deg F   " + "%2.1f" % humidT.get_currentValue() + "%  " + str(int(lightT.get_currentValue())/82) + ' PAR')
                        i = i + 1
                        time.sleep(1)
        else:
        ...
        if lightTM.isOnline() and humidTM.isOnline() is True:
        	i = 1
        	for i in range(1,6):
                	print('%2.2f' % tempTM.get_currentValue() + "deg F   " + "%2.1f" % humidTM.get_currentValue() + "%  " + str(int(lightTM.get_currentValue())/82) + ' PAR')
                        i = i + 1
                        time.sleep(1)
        else:
                print('No devices found!')
        ...
        if lightML.isOnline() and humidML.isOnline() is True:
        	i = 1
                for i in range(1,6):
                	print('%2.2f' % tempML.get_currentValue() + "deg F   " + "%2.1f" % humidML.get_currentValue() + "%  " + str(int(lightML.get_currentValue())/82) + ' PAR')
                        i = i + 1
                        time.sleep(1)
        else:
        	print('No devices found!')
        ...
        if lightL.isOnline() and humidL.isOnline() is True:
        	i = 1
                for i in range(1,6):
                	print('%2.2f' % tempL.get_currentValue() + "deg F   " + "%2.1f" % humidL.get_currentValue() + "%  " + str(int(lightL.get_currentValue())/82) + ' PAR')
                        i = i + 1
                        time.sleep(1)
        else:
                print('No devices found!')
